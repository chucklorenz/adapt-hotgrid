define([
    ...
    var HotgridView = ComponentView.extend({
        ...
        setClassTemplate: function() {
            var _classes = this.model.get('_classes');
            if( _classes === '' || !_classes ) return;
            var tokens = _classes.split(" ");
            _.find(tokens, function (className) {
                if (!Handlebars.partials[className]) return;
                this.model.set('_classTemplate', className);
            }, this);
        },
